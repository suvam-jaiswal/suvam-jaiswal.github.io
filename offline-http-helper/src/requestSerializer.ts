...
export class RequestSerializer {
  ...
  static async serializeBody(body: unknown): Promise<SerializedBody> {
    if (!isDefined(body)) {
      return { kind: 'none' };
    }
    ...
    if (ArrayBuffer.isView(body)) {
      ...
      const cloned = typeof (viewBuffer as ArrayBuffer).slice === 'function'
        ? (viewBuffer as ArrayBuffer).slice(0)
        : viewBuffer;
      return { kind: 'arrayBuffer', value: cloned as ArrayBuffer };
    }
  ...
  static async deserializeBody(body: SerializedBody): Promise<BodyInit | undefined> {
    switch (body.kind) {
      case 'none':
        return undefined;
      case 'text':
  ...
  static async serializeRequestInit(init: RequestInit = {}): Promise<SerializedRequestInit> {
    return {
      headers: this.serializeHeaders(init.headers),
      body: await this.serializeBody(init.body as unknown),
  ...
  static async deserializeRequestInit(serialized: SerializedRequestInit): Promise<RequestInit> {
...
async function serializeFormData(formData: FormData): Promise<SerializedFormDataEntry[]> {
  ...
  for (const [name, value] of formData.entries()) {
    if (typeof value === 'string') {
      ...
      const blobValue: SerializedFormDataEntry['value'] = {
        kind: 'blob',
        value: buffer,
      };
      if (value.type) {
...
function getName(value: Blob): string | undefined {
  if ('name' in value && typeof (value as File).name === 'string') {
