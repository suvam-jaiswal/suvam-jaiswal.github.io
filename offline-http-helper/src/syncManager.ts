...
export class HttpSyncManager {
  ...
  async replayPending(): Promise<void> {
    ...
    this.syncing = true;
    const stats: SyncStats = { processed: 0, succeeded: 0, failed: 0, remaining: 0 };
    try {
  ...
  async retryErroredRequest(options: ReplayErroredRequestOptions): Promise<ReplayResult | undefined> {
    const stored = await this.queue.get(options.requestId);
    if (!stored) {
      return undefined;
    }

    const merged: QueuedRequestPayload = {
      ...stored,
  ...
  private async executeRequest<T = unknown>(request: QueuedRequestPayload): Promise<HttpResponse<T>> {
    ...
    const parsed = await parseResponse<T>(response, request.responseType, 'replay', request.id);
    if (!response.ok) {
